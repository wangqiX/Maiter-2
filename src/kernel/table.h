...
struct IterateKernel : public IterateKernelBase {
  ...
  virtual void read_data(string& line, K& k, D& data, int& data_property,map<int,float>& mirrors,int& master) = 0;
  virtual void init_c(const K& k, V& delta,D& data, int data_property) = 0;//delta
//  virtual void init_sbuffer(const K& k,V& SBuffer){}
  virtual const V& default_v() const = 0;//默认值
  virtual void init_v(const K& k, V& v, D& data, int& data_property) = 0;//value
  virtual void accumulate(V& a, const V& b) = 0;//累积操作
  virtual void offset(V& a, const V& b)=0;//抵消操作,pagerank:-  sssp:if a==b 就
  virtual void process_delta_v(const K& k, V& dalta,V& value, D& data){} //,int& data_property,D &mirrors
  virtual void priority(V& pri, const V& value, const V& delta) = 0;
  virtual void g_func(const K& k,const V& delta,const V& value, const D& data, int& data_property, vector<pair<K, V> >* output) = 0;
};
...
struct TermChecker : public TermCheckerBase {
    ...
    //virtual bool terminate(vector<double> local_reports,double time)=0;
};
  ...
  struct Diff : public TermChecker<K, V> {
    ...
    double estimate_prog(LocalTableIterator<K, V>* statetable){
        ...
        while(!statetable->done()){
            ...
            if(statetable->value2() != defaultv){//&& statetable->ismaster()  非master顶点value值都=0 应该,所以这里
               if(partial_curr>INT_MAX){partial_curr= 913100; LOG(INFO) <<"error"<<endl;break;}
               partial_curr += static_cast<double>(statetable->value2());
               //LOG(INFO) <<"key="<<statetable->key()<<" partial_curr="<<partial_curr<<"  value2="<<statetable->value2()<<"\n";
            }
        }
        //LOG(INFO) <<"partial_curr="<< partial_curr<<"\n";
       // if(partial_curr!=0)partial_curr= 913000;
        return partial_curr;
    }
...
public:
  ...
  MarshalBase *value4_marshal;
  //MarshalBase *value5_marshal;
...
struct UntypedTable {
  ...
  virtual void update_str(const StringPiece &k, const StringPiece &v1, const StringPiece &v2, const StringPiece &v3, const StringPiece &v4, const StringPiece &v5) = 0;
};
...
public:
  ...
  virtual int getF4(const K &k){return 0;};
  virtual map<int,V1> getF5(const K &k)=0;//{V3 v;return v;};
  virtual int  getmaster(const K& k)=0;
  virtual int get_size(const int shard)=0;//获取顶点的个数
  //virtual bool get_ismaster(const K &k)=0;
  virtual ClutterRecord<K, V1, V2, V3> get(const K &k) = 0;
  virtual void put(const K &k, const V1 &v1, const V2 &v2, const V3 &v3, const int &v4, const map<int,V1> &v5,const int &master) = 0;//,const int &v4,const V3 &v5
  virtual void updateF1(const K &k, const V1 &v,const int& shard) = 0;
  virtual void updateF2(const K &k, const V2 &v,const int& shard) = 0;
  virtual void updateF3(const K &k, const V3 &v) = 0;
  virtual void updateF4(const K &k, const int &v)=0;
  virtual void updateF5(const K &k, const V3 &v)=0;
  virtual void accumulateF1(const K &k, const V1 &v,const int& source,const int& shard,const bool& is_SendMessage) = 0; //4 TypeTable
  void accumulateDelta(const K &k,const V1& v,const int& shard){}
  virtual void accumulateSBuffer(const K& k,const int& p,const V1 &v){}
  virtual void subtractMBuffer(const K& k,const V1 &v,const int& part){}
  virtual void accumulateF2(const K &k, const V2 &v,const int& shard) = 0;
  virtual void accumulateF3(const K &k, const V3 &v) = 0;
  virtual void accumulateF4(const K &k, const int &v)=0;
  virtual void accumulateF5(const K &k, const V3 &v)=0;
  ...
  virtual string get_str(const StringPiece &s) {
    ...
    v4marshal()->marshal(getF4(k), &f4);
    //v5marshal()->marshal(getF5(k), &f5);
    return f1+f2+f3+f4;
  }
   //应该是用于容错??
  virtual void update_str(const StringPiece &kstr, const StringPiece &vstr1, const StringPiece &vstr2, const StringPiece &vstr3, const StringPiece &vstr4, const StringPiece &vstr5){
    K k; V1 v1; V2 v2; V3 v3;int v4;map<int,V1> v5;;//瞎写的
    kmarshal()->unmarshal(kstr, &k);
    v1marshal()->unmarshal(vstr1, &v1);
    v2marshal()->unmarshal(vstr2, &v2);
    v3marshal()->unmarshal(vstr3, &v3);
    v4marshal()->unmarshal(vstr4, &v4);
    //v5marshal()->unmarshal(vstr5, &v5);
    put(k, v1, v2, v3,v4,v5,1);//待规范
  }
...
protected:
  ...
  virtual Marshal<int> *v4marshal() = 0;
  //virtual Marshal<V3> *v5marshal() = 0;
...
struct TypedTableIterator : public TableIterator {
  ...
  virtual const K& key() = 0;
  virtual V1& value1() = 0;//value
  virtual V2& value2() = 0;//delta
  virtual V3& value3() = 0;//link
  virtual int& value4() = 0;//length
  virtual map<int,V1>& value5() = 0;//mirror
  virtual int& master() = 0;//master
  virtual int& position() = 0;
  virtual float priority(){return float(0);}
  virtual void updateF1F2(bool is_master,vector<pair<int, V1> >* output_mirrors){}
...
//virtual void updateF1F2(bool is_master){
//    
//}
//  
...
private:
  vector<pair<K, V1> > decodedeque;
  typename vector<pair<K, V1> >::iterator intit;
};
