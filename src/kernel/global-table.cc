...
DEFINE_double(snapshot_interval, 1, "");
//DEFINE_int32(snapshot_interval, 1, "");//99999999
...
void MutableGlobalTableBase::TermCheck() {
   // VLOG(0) << "FLAGS_snapshot_interval=" << FLAGS_snapshot_interval<<endl;
...
void MutableGlobalTableBase::termcheck() {
    ...
    if(helper()){
        ...
        //VLOG(0) << "发送终止检测,"<<"snapshot_index="<<snapshot_index<<";total_updates="<<total_updates<<";total_current="<<total_current;
    }
...
void MutableGlobalTableBase::SendUpdates() {
 ...
 // static double communications=0;
  ...
  //VLOG(0)<<"worker"<<this->info_.helper->id()<<"上的communications="<<communications<<endl;
