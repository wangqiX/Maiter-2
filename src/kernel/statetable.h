...
#pragma pack(push, 1)
  ...
  struct Bucket {
    ...
    int v4;//所有邻居的长度
    int master;
    map<int,V1> v5;//mirrors p_id,
    V1 priority;
    bool in_use;
    //在
    //V1 s_buffer;//master发送给mirror delta的缓冲区.在master node上  //注释掉
    //vector<V1> m_buffer;//size=mirror的个数    //机器的个数
  };
#pragma pack(pop)
  ...
  struct Iterator : public TypedTableIterator<K, V1, V2, V3> {
	...
	virtual ~Iterator(){}
        ...
        bool Next() {
          ...
          } while (pos < parent_.size_ && ( !parent_.buckets_[pos].in_use));//parent_.buckets_[pos].v3.size()==0||
        ...
        int& value4() { return parent_.buckets_[pos].v4; }
        map<int,V1>& value5() { return parent_.buckets_[pos].v5; }
        K& master(){ return parent_.buckets_[pos].master; }
        
        void updateF1F2(bool is_master,vector<pair<int, V1> >* output_mirrors){
           parent_.total_updates++;
           Bucket& bucket=parent_.buckets_[pos];
           if(is_master){
               bucket.v2+=bucket.v1;//如果是master顶点,则将该顶点的value值累加
               typename std::map<int,V1>::iterator iter2;
               for(iter2=bucket.v5.begin();iter2!=bucket.v5.end();++iter2){
                       int part=iter2->first;
                       //pagerank
                       output_mirrors->push_back(make_pair(part,bucket.v1-iter2->second));
                       //sssp
                       //if(bucket.v1==bucket.v5[i].second)continue;
                       //output_mirrors->push_back(make_pair(part,bucket.v1));
                       iter2->second=defaultv;
               } 
           }
           bucket.v1=defaultv;//delta值清空
           ((IterateKernel<K, V1, V3>*)parent_.info_.iterkernel)->priority( bucket.priority,bucket.v2,bucket.v1);//重置priority
        }
...
int& position() { return pos; }
        ...
        int& value4() { return parent_.buckets_[scheduled_pos[pos]].v4; }
        map<int,V1>& value5() { return parent_.buckets_[scheduled_pos[pos]].v5; }
        int& master(){ return parent_.buckets_[scheduled_pos[pos]].master; }
   ...
   struct EntirePassIterator : public TypedTableIterator<K, V1, V2, V3>, public LocalTableIterator<K, V2> {
	...
	virtual ~EntirePassIterator(){}
        ...
        map<int,V1>& value5() { return parent_.buckets_[pos].v5; }
        K& master(){ return parent_.buckets_[pos].master; }
        float priority(){return parent_.buckets_[pos].priority;}
        //int size(){return parent_.buckets_.entries_}
       // void updateF1F2(bool is_master){}
  ...
  virtual int get_size(const int shard);
  //bool get_ismaster(const K& k);
  ...
  void accumulateF1(const K& k, const V1& v,const int& source,const int& shard,const bool& is_SendMessage);
  ...
  void accumulateSBuffer(const K& k,const int& p, const V1& v);
  void subtractMBuffer(const K& k,const V1 &v,const int& part);
  ...
  TableIterator *get_iterator(TableHelper* helper, bool bfilter) {
      ...
      static int step=0;
      if(++step%1==0){
         helper->FlushUpdates();//发送消息
         boost::this_thread::sleep( boost::posix_time::seconds(0.1) );//sleep形参如果是负的,那么就是
         helper->HandlePutRequest();//接受消息
      }

      Iterator* iter = new Iterator(*this, true);//
      //iter->b_no_change=false;//for test 
      int trial = 0;
      while(iter->b_no_change){
          VLOG(0) << "wait for put";//VLOG(0)<<"6666666666666"<<endl;
          delete iter;
          helper->FlushUpdates();
     ...
     // LOG(INFO) <<"worker"<<helper->id()<<":getiteratortime="<<getiteratortime<<endl;
      return iter;
  }
 ...
 // Marshal<vector<pair<int,V1> > >* v5marshal() { return ((Marshal<vector<pair<int,V1> > >*)info_.value5_marshal); }
private:
...
void StateTable<K, V1, V2, V3>::serializeToSnapshot(const string& f, long* updates, double* totalF2) {
  ...
  delete entireIter;
  *updates = total_updates;//VLOG(0)<<"total_updates="<<total_updates<<endl;
  *totalF2 = total_curr;
}
...
void StateTable<K, V1, V2, V3>::resize(int64_t size) {
 ...
 // LOG(INFO) << "Rehashing.vd.. " << entries_ << " : " << size_ << " -> " << size;
  ...
  for (int i = 0; i < old_b.size(); ++i) {
    if (old_b[i].in_use) {    
      put(old_b[i].k, old_b[i].v1, old_b[i].v2, old_b[i].v3,old_b[i].v4,old_b[i].v5,old_b[i].master);
    }
  }
  //LOG(INFO) << "resize... " << Now()-start_time;
}
...
template <class K, class V1, class V2, class V3>
V1 StateTable<K, V1, V2, V3>::getF1(const K& k) {//未被调用
...
template <class K, class V1, class V2, class V3>
V2 StateTable<K, V1, V2, V3>::getF2(const K& k) {////未被调用
  int b = bucket_for_key(k);
  //The following key display is a hack hack hack and only yields valid
  //results for ints.  It will display nonsense for other types.
  CHECK_NE(b, -1) << "No entry for requested key <" << *((int*)&k) << ">";
...
template <class K, class V1, class V2, class V3>
V3 StateTable<K, V1, V2, V3>::getF3(const K& k) {//未被调用
  int b = bucket_for_key(k);
  //The following key display is a hack hack hack and only yields valid
  //results for ints.  It will display nonsense for other types.
  CHECK_NE(b, -1) << "No entry for requested key <" << *((int*)&k) << ">";
...
template <class K, class V1, class V2, class V3>
int StateTable<K, V1, V2, V3>::getF4(const K& k) {//使用 length
  int b = bucket_for_key(k);
  CHECK_NE(b, -1) << "No entry for requested key <" << *((int*)&k) << ">";

  return buckets_[b].v4;
}

template <class K, class V1, class V2, class V3>
map<int,V1> StateTable<K, V1, V2, V3>::getF5(const K& k) {//使用  mirrors
  int b = bucket_for_key(k);
  CHECK_NE(b, -1) << "No entry for requested key:master mirrors<" << *((int*)&k) << ">";

  return buckets_[b].v5;
}
template <class K, class V1, class V2, class V3>
int StateTable<K, V1, V2, V3>::getmaster(const K& k) {//使用 
  int b = bucket_for_key(k);
  CHECK_NE(b, -1) << "No entry for requested key:master mirrors　worker="<<this->info_.helper->id()<<" key="<<k<< ">";
  return buckets_[b].master;
}
template <class K, class V1, class V2, class V3>
int StateTable<K, V1, V2, V3>::get_size(const int shard){
    return(this->entries_);
}
//bool StateTable<K, V1, V2, V3>::get_ismaster(const K& k) {//使用
//  int b = bucket_for_key(k);
//  //CHECK_NE(b, -1) << "No entry for requested key:master mirrors<" << *((int*)&k) << ">";
//  if(b==-1)return false;//如果该顶点不存在的话,肯定不是master顶点
//  return buckets_[b].is_master;
//}
...
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::updateF1(const K& k, const V1& v,const int& shar) {
    int b = bucket_for_key(k);
...
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::updateF2(const K& k, const V2& v,const int& shar) {
    int b = bucket_for_key(k);
...
template <class K, class V1, class V2, class V3>//用不到
...
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::updateF4(const K& k, const int& v) {

}
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::updateF5(const K& k, const V3& v) {

}
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::accumulateF1(const K& k, const V1& v,const int& sourc,const int& shar,const bool& is_SendMessage) {
 ...
 //VLOG(0) <<"顶点"<<endl;
  ...
  if(b!=-1){
    //if(buckets_[b].v3.size()==0)return;//出度为零的mirror顶点拒绝接受delta //maiter+_0.2
    ((IterateKernel<K, V1, V3>*)info_.iterkernel)->accumulate(buckets_[b].v1, v); 
//    if(shar==0){//for test part0
//        LOG(INFO) <<"顶点"<<buckets_[b].k<<":"<<buckets_[b].v1<<endl;
//    }
    //
    ((IterateKernel<K, V1, V3>*)info_.iterkernel)->priority(buckets_[b].priority, buckets_[b].v2, buckets_[b].v1);//LOG(INFO) <<"worker"<<shar<<"222"<<endl;
  }
}

template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::accumulateF2(const K& k, const V2& v,const int& shar) {
  int b = bucket_for_key(k);
...
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::accumulateF4(const K& k, const int& v) {

}
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::accumulateF5(const K& k, const V3& v) {

}
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::accumulateSBuffer(const K& k,const int& p, const V1& v) {
  int b = bucket_for_key(k);
  CHECK_NE(b, -1) << "No entry for requested key <" << *((int*)&k) << ">";
  if(b!=-1){
     ((IterateKernel<K, V1, V3>*)info_.iterkernel)->accumulate(buckets_[b].v5[p], v);
  }
}
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::subtractMBuffer(const K& k, const V1& v,const int& part) {
//   int b = bucket_for_key(k);
//
//  //CHECK_NE(b, -1) << "No entry for requested key <" << *((int*)&k) << ">";
//  if(b!=-1){
//     ((IterateKernel<K, V1, V3>*)info_.iterkernel)->accumulate(buckets_[b].m_buffer[part], -1*v);
//  }
}
...
template <class K, class V1, class V2, class V3>
void StateTable<K, V1, V2, V3>::put(const K &k, const V1 &v1, const V2 &v2, const V3 &v3, const int &v4, const map<int,V1> &v5,const int &master) {
  int start = bucket_idx(k);//double start_time= Now();
  int b = start;
  bool found = false;
  //if(k==92283370)VLOG(0)<<"1master="<<master<<" mirror.size="<<v5.size()<<endl;
  do {
    if (!buckets_[b].in_use) {
      break;
    }
  ...
  //put_time+=Now()-start_time;
  // Inserting a new entry:
  if (!found) {
    if (entries_ >= size_ ) {     //doesn't consider loadfactor, the tablesize is pre-defined 
        VLOG(0) << "resizing... " << size_ << " : " << (int)(1 + size_ * 2) << " entries "<< entries_;
        //VLOG(0) << "put_time= " <<put_time;
        resize((int)(1 + size_ * 2));
        put(k, v1, v2, v3,v4,v5,master);
    } else {
     // if(k==92283370)VLOG(0)<<"2master="<<master<<" mirror.size="<<v5.size()<<endl;
      buckets_[b].in_use = 1;
      buckets_[b].k = k;
      buckets_[b].v1 = v1;
      buckets_[b].v2 = v2;
      ...
      buckets_[b].v4 = v4;
      buckets_[b].v5 = v5;
      buckets_[b].master = master;
      ((IterateKernel<K, V1, V3>*)info_.iterkernel)->priority(buckets_[b].priority, buckets_[b].v2, buckets_[b].v1);
      ++entries_;
      // if(k==92283370)VLOG(0)<<"3master="<<buckets_[b].master<<" mirror.size="<< buckets_[b].v5.size()<<endl;
    }
  } else {//VLOG(0) << "2222222222... ";
    // Replacing an existing entry
      //if(k==92283370)VLOG(0)<<"3master="<<master<<" mirror.size="<<v5.size()<<endl;
    buckets_[b].v1 = v1;
    buckets_[b].v2 = v2;
    buckets_[b].v3 = v3;
    buckets_[b].v4 = v4;
