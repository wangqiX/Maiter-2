...
//DEFINE_int32(convergence_rate_interval, 2, "");//每隔termcheck_interval时间段的2倍,进行一次检测
DEFINE_double(termcheck_interval, 1, "");
//DEFINE_int32(termcheck_interval, 1, "");//(name,value,text)  1
...
Master::Master(const ConfigData &conf) :tables_(TableRegistry::Get()->tables()){
//  convergence_rate_interval=5;//
//  convergence_rate_number=0;
//  sum_last=916428;
//  
...
//   bool bterm=false;
//  if(total_F2!=0) bterm=true;//for test 只要进行了收到worker的消息就终止
  //##收敛进度检测
        string file = StringPrintf("result/pagerank/convergence_rate");  //the output path
        fstream File;  
        File.open(file.c_str(), ios::out|ios::app);
        File << barrier_timer->elapsed() <<" "<< (916428-total_F2)<<endl;
        File.clear();
        File.close();
...
int Master::reap_one_task() {//检测到一个消息成功
...
void Master::barrier() {
  ...
  iter++;
  //LOG(INFO) <<"数据块数="<<current_run_.shards.size();
  ...
  while (finished_ < current_run_.shards.size()) {
    ...
    if (!terminated_ && Now() - last_termcheck_ > FLAGS_termcheck_interval) {
    	bterm = termcheck();//VLOG(0) << "FLAGS_termcheck_interval=" << FLAGS_termcheck_interval<<endl;
        last_termcheck_ = Now();
        VLOG(2) << "term ? " << bterm;
